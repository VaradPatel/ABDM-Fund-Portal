...
public interface ImplementInsuCalcRepo extends JpaRepository<ImplementInsuCalc, Integer> {
    @Query(value ="Select * from implement_insurance_hybrid_calc where request_id= :requestId  and role_id= :roleId ORDER BY created_at DESC LIMIT 1", nativeQuery = true)
