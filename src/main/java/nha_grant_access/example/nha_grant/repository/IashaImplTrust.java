...
public interface IashaImplTrust extends JpaRepository<AashaImplTrust, Integer> {
    @Query(value ="Select * from asha_impl_trust where request_id= :requestId ORDER BY created_at DESC LIMIT 1", nativeQuery = true)
    Optional<AashaImplTrust> findByRequestId(String requestId);
}
